		...
		method: 'POST', path: '/gethappy', config: {
			handler: function(request, reply) {
				...
				if (session) {
					...
					data.User.find(parseInt(userid)).success(function(user) {
						data.Memory.create({ description: desc, date: date }).success(function(newMemory) {
							newMemory.setUser(user).success(function() {
